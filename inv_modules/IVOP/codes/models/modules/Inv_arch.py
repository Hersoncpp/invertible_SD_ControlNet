...
# Import new modules: Pixel Shuffle, HAAR, and Invertible 1x1 Conv
...
class InvSubSampleNetD(nn.Module):
    # subsample module like haar and pixel shuffle and go down to 1/4 size then come thru an invertible Conv1x1
    def __init__(self, channel_in=3, channel_out=3, subnet_constructor=None, subsampler=None, use_inv_conv1x1=True, block_num=[], down_num=2, non_inv_block = None, fusion_submodule_type = None, save_intermediate=False):
        super(InvSubSampleNetD, self).__init__()

        self.save_intermediate = save_intermediate
        self.intermediate_outputs = {}
        self.subsampler = subsampler
        if subsampler == 'pixel_shuffle':
            self.subsampler = PixelShuffle(subsampler_factor = 2)
        elif subsampler == 'haar':
            self.subsampler = HaarTransform()
        else:
            self.subsampler = None
        if use_inv_conv1x1:
            self.inv_conv1x1 = InvertibleConv1x1(channel_in)
        else:
            self.inv_conv1x1 = None
        operations_cover = []
        operations_secret = []
        operations_final = []
        current_channel = channel_in
