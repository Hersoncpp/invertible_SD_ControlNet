...
class IRNModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        self.ref_L =  self.ref_L[:,:,:self.ref_L.shape[2]//4*4,:self.ref_L.shape[3]//4*4]
        self.real_H =  self.real_H[:,:,:self.real_H.shape[2]//4*4,:self.real_H.shape[3]//4*4]
    ...
    def optimize_parameters(self, step):
        ...
        ######   
        # jpeg quality
        # LR_ = (255*LR).permute(0,2,3,1).detach().clone().cpu().numpy().astype(np.uint8)
        # LR_J = np.zeros_like(LR_)
        # for i in range(LR_.shape[0]):
        #     LR_J[i,:,:,:] = cv2.imdecode(np.array(cv2.imencode('.jpg', LR_[i,:,:,::-1])[1]) , cv2.IMREAD_COLOR)[:,:,::-1]
        # LR_J = torch.from_numpy(LR_J/255.).to(self.device).permute(0,3,1,2).float() 
        # LR = LR + (LR_J - LR).detach()
    ...
    def test(self):
        ...
        zshape = [Lshape[0], 3, Lshape[2], Lshape[3]]
        ...
        with torch.no_grad():
            ...
            forw_L = self.netG(x=self.input)
            
            dump = forw_L[:, 3:, :, :]
            print('dump', torch.mean(dump), torch.max(dump), torch.min(dump))
            self.forw_L = self.Quantization(forw_L[:, :3, :, :])
            # if torch.randn((1))<0.05:
            #   LR_ = (255*dump).permute(0,2,3,1).detach().clone().cpu().numpy().astype(np.uint8)
            #   cv2.imwrite('./tmp3.jpg', LR_[0], [cv2.IMWRITE_JPEG_QUALITY, 90])            
            #####
            # LR_ = (255*self.forw_L).permute(0,2,3,1).detach().clone().cpu().numpy().astype(np.uint8)
            # cv2.imwrite('./tmp3.jpg', LR_[0], [cv2.IMWRITE_JPEG_QUALITY, 100])
            # LR_J = cv2.imread('./tmp3.jpg')
            # self.forw_L = torch.from_numpy(LR_J/255.).unsqueeze(0).to(self.device).permute(0,3,1,2).float() 
            # LR_ = (255*self.forw_L).permute(0,2,3,1).detach().clone().cpu().numpy().astype(np.uint8)
            # LR_J = np.zeros_like(LR_)
            # # print(LR_.shape[0])
            # for i in range(LR_.shape[0]):
            #     LR_J[i,:,:,:] = cv2.imdecode(np.array(cv2.imencode('.jpg', LR_[i,:,:,::-1])[1]) , cv2.IMREAD_COLOR)[:,:,::-1]
            # LR_J = torch.from_numpy(LR_J/255.).to(self.device).permute(0,3,1,2).float() 
            # self.forw_L = self.forw_L + (LR_J - self.forw_L).detach()
            # self.forw_L = LR
#                  
            # y_forw = torch.cat((self.forw_L, 0 * self.gaussian_batch(zshape)), dim=1)
            y_forw = torch.cat((self.forw_L,   self.gaussian_batch(zshape)), dim=1)
